...
export default function Navbar() {
  ...
  const isModulPage = pathname.startsWith('/modul');
      ...
      <form className="hidden md:flex order-3 md:order-2 w-full md:w-auto flex-grow max-w-full md:max-w-4xl mx-auto">
        ...
        <div className="relative w-full">
          ...
          <input
            ...
            placeholder={isModulPage ? "Cari modul di sini..." : "Mau belajar apa hari ini?"}
            value={isModulPage ? searchQuery : ''}
            onChange={isModulPage ? (e) => setSearchQuery(e.target.value) : undefined}
            readOnly={!isModulPage}
            required
          />
        </div>
