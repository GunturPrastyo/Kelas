...
export default function ModulDetailPage() {
    ...
    const questionHtml = useMemo(() => {
        return currentQuestionForModal ? { __html: currentQuestionForModal.questionText } : undefined;
    ...
    useEffect(() => {
        const loadSettings = () => {
            ...
            if (storedFontSize) {
                ...
                // Default ke 14px jika di mobile (< 768px) dan belum ada pengaturan
        ...
        if (hash) {
            ...
            for (const topic of modul.topics) {
                if (topic.materi && topic.materi.subMateris && topic.materi.subMateris.some(sub => sub._id === hashId)) {
                    ...
                    }, 600); // Delay ditingkatkan agar animasi accordion selesai
                    return;
                }
            }
        ...
        return (
            <div className="w-full max-full fixed inset-0 bg-black bg-opacity-60 z-50 flex justify-center items-center p-4">
                <div className="bg-white dark:bg-gray-800 rounded-xl shadow-2xl w-full max-w-6xl max-h-[90vh] flex flex-col overflow-hidden topic-test-modal">
                    ...
                    <main className="flex-1 p-6 overflow-y-auto" ref={questionModalRef}>
                        {testResult ? (
            ...
            rounded-full shadow-sm tracking-wide">
                                                    ...
                                                    <div className="flex items-center gap-3">
                                                        ...
                                                        <a
                                                            ...
                                                            onClick={(e) => {
                                                                ...
                                                                // Cari topik parent dari subMateriId agar accordion bisa dibuka
                                                                const parentTopic = modul.topics.find(t => 
                                                                    t.materi?.subMateris.some(sm => sm._id === sub.subMateriId)
                                                                );

                                                                if (parentTopic) {
                                                                    setOpenTopicId(parentTopic._id);
                                                                    router.push(`/modul/${modul.slug}#${sub.subMateriId}`);
                                                                    setTimeout(() => {
                                                                        document.getElementById(sub.subMateriId)?.scrollIntoView({ behavior: 'smooth', block: 'start' });
                                                                    }, 600);
                                                                } else {
                                                                    router.push(`/modul/${modul.slug}#${sub.subMateriId}`);
                                                                }
                    ...
                    {/* Navigation Sidebar (Only show during test, not result) */}
                    {!testResult && (
                        <aside className={`
                            bg-slate-50 dark:bg-gray-900/50 border-l border-gray-200 dark:border-gray-700 p-6 overflow-y-auto shrink-0
                            lg:block lg:w-80 lg:static
                        ...
                        `}>
                                ...
                                {activeTest.questions.map((q, idx) => {
                                    ...
                                    return (
                                        <button
                                            ...
                                            className={`h-10 w-full rounded-lg text-sm font-semibold transition-all flex items-center justify-center
                            ...
                            <div className="mt-6 space-y-2">
                                <div className="flex items-center gap-2 text-xs text-gray-500 dark:text-gray-400">
                                    <span className="w-3 h-3 rounded bg-blue-600"></span> Terjawab
                                </div>
                                <div className="flex items-center gap-2 text-xs text-gray-500 dark:text-gray-400">
                                    <span className="w-3 h-3 rounded bg-white border border-gray-300 dark:bg-gray-800 dark:border-gray-600"></span> Belum Dijawab
                                </div>
                            </div>
                        </aside>
                    )}
